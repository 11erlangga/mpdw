...
<ul>
  <li> `View()` : menampilkan data dalam bentuk tabel
  <li> `str()` : menampilkan struktur data
  <li> `dim()` : menampilkan dimensi data
</ul>
...
<ol>
  <li> `Time Series:`
       Output ini menunjukkan bahwa data telah dikenali R sebagai objek deret waktu (ts).
  <li> `Start = 1`
       Titik awal deret waktu ada pada observasi pertama. Karena tidak diberikan parameter tambahan (`start`, `end`, atau `frequency`), R otomatis menganggap indeks pertama adalah 1.
  <li> `End = 50`
       Data berakhir pada observasi ke-50. Artinya jumlah data = 50 titik.
  <li> `Frequency = 1`
       Frekuensi menunjukkan berapa banyak periode dalam satu unit waktu.
       <ul>
        <li> `frequency = 1` → dianggap data tahunan (setiap titik = 1 tahun).
        <li> Jika datanya bulanan, biasanya dipakai `frequency = 12`.
        <li> Jika data kuartalan, dipakai `frequency = 4`.
      </ul>
      Karena tidak diatur parameter `frequency`, default-nya jadi 1.
  <li> `Deret Angka [1] ... [40] ...`
       Ini adalah nilai dari variabel `Yt` (yang diubah jadi time series). Ditampilkan dalam bentuk vektor, dipotong per 13 angka agar muat di layar.
</ol>
...
# Kalau mau menyimpan plot
# png("plot_data2.png")
# ts.plot(data2.ts, xlab="Time Period ", ylab="Reading",
#        main = "Time Series Plot")
# points(data2.ts)
# dev.off()
```
...
Eksplorasi data juga dapat dilakukan menggunakan package `ggplot2` dengan terlebih dahulu memanggil *package* `ggplot2`.
...
<ol>
  <li> fungsi `ses()` dari *packages* `forecast`
  <li> fungsi `HoltWinters` dari *packages* `stats`
</ol>
...
<ul>
  <li> `y` : nilai data deret waktu
  <li> `alpha` : parameter pemulusan utama (0 – 1), mengatur bobot data terbaru vs data lama.
  <li> `beta` : parameter pemulusan tren.
  <li> `gamma` : parameter pemulusan musiman.
  <li> `h` : jumlah periode ke depan yang ingin diramalkan (forecast horizon).
</ul>
...
Untuk mendapatkan gambar hasil pemulusan pada data latih dengan fungsi `ses()`, perlu digunakan fungsi `autoplot()` dan `autolayer()` dari *packages* `ggplot2` .
...
Nilai parameter $\alpha$ dari kedua fungsi dapat dioptimalkan menyesuaikan dari *error*-nya, error pada data training (*in-sample error*)), paling minimumnya. Caranya adalah dengan membuat parameter $\alpha =$ `NULL` .
...
```{r}
MSEfull <-
  matrix(c(MSEtesting1, MSEtesting2, MSEtestingopt, MSEtestingdes1, MSEtestingdes2, MSEtestingdesopt),
         nrow=3, ncol=2)
